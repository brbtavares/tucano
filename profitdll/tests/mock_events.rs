...
async fn mock_emits_synthetic_events() {
    ...
    let mut rx = connector.initialize_login("act", "user", "pass").await.unwrap();
    ...
    for _ in 0..10 {
        if let Some(evt) = rx.recv().await {
            match evt {
                ...
                CallbackEvent::PriceBookOffer { .. } | CallbackEvent::OfferBookBid { .. } => got_book = true,
                CallbackEvent::OrderUpdated { .. } | CallbackEvent::OrderSnapshot { .. } => got_order = true,
                _ => {}
            }
        }
        if got_progress && got_book && got_order { break; }
    }
    assert!(got_progress && got_book && got_order, "missing synthetic events: progress={got_progress} book={got_book} order={got_order}");
}

...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let creds = match Credentials::from_env() {
        Ok(c) => c,
        Err(e) => {
            eprintln!("[live_minimal] Credenciais ausentes: {e}");
            return Ok(());
        }
    };
    ...
    loop {
        tokio::select! { evt = rx.recv() => { match evt { Some(e)=>{ println!("evt[{count}]: {e:?}"); count+=1; if count>=20 { break; }}, None=>break } } _ = &mut timeout => { println!("[live_minimal] Timeout"); break; } }
    }
    println!("[live_minimal] Encerrado (eventos: {count}).");
    Ok(())
}

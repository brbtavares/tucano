...
impl TearSheetAssetGenerator {
    /// Initialise a [`TearSheetAssetGenerator`] from an initial `AssetBalance`.
    pub fn init(initial: &AssetBalance<impl std::fmt::Debug>) -> Self {
        Self {
            balance_now: Some(initial.balance),
            drawdown: DrawdownGenerator::init(initial.balance.total, initial.time_exchange),
    ...
    /// Reset the internal state, using a new starting `AssetBalance` as seed.
    pub fn reset(&mut self, balance_start: &AssetBalance<impl std::fmt::Debug>) {

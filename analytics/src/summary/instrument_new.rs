...
impl TearSheetGenerator {
    ...
    {
        let duration = interval.duration_ns() as i64;
        TearSheet {
            pnl: self.pnl_returns.pnl_raw,
            pnl_return: RateOfReturn::new(
                self.pnl_returns.pnl_raw,
                TimeDelta::nanoseconds(duration),
            ),
            sharpe_ratio: SharpeRatio::new(
                self.pnl_returns.total.mean,
                self.pnl_returns.total.std_deviation,
                Decimal::ZERO, // risk-free return
                TimeDelta::nanoseconds(duration),
            ),
            sortino_ratio: SortinoRatio::new(
                self.pnl_returns.total.mean,
                self.pnl_returns.losses.std_deviation,
                Decimal::ZERO, // risk-free return
                TimeDelta::nanoseconds(duration),
            ),
            ...
            profit_factor: ProfitFactor::new(
                self.pnl_returns.wins.sum,
                self.pnl_returns.losses.sum.abs(),
            ),
            win_rate: WinRate::new(
                self.pnl_returns.wins.count,

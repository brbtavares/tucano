...
impl DrawdownGenerator {
    /// Initialise a [`DrawdownGenerator`] from an initial value and time.
    ...
    /// Updates the internal [`DrawdownGenerator`] state using the latest value and time.
    ///
    ...
    pub fn update(&mut self, value: Decimal, time: DateTime<Utc>) -> Option<Drawdown> {
        ...
        if value > peak {
            ...
            let drawdown_current = (peak - value).checked_div(peak);
